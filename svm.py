...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    :param dataMatIn:
    :param classLabels:
    :param C:               常数C（控制最大化间隔、保证大部分点的函数间隔小于1.0两个条件的权重）
    :param toler:           容错率
    ...
    b = 0
    m, n = shape(dataMatrix)                    # 100行, 2列
    alphas = mat(zeros((m, 1)))
    iter = 0
    while iter < maxIter:
        alphaPairsChanged = 0                   # 记录alpha是否已进行优化
        for i in range(m):                      # 逐行样本处理
            fXi = float(multiply(alphas, labelMat).T * (dataMatrix * dataMatrix[i, :].T)) + b   # 预测类别（multiply：对应元素相乘）
            Ei = fXi - float(labelMat[i])       # 预测与实际误差值（误差大可对该数据实例的alpha值优化）

            # 不符合KTT条件：优化alpha
            if (labelMat[i] * Ei < -toler and alphas[i] < C) or (labelMat[i] * Ei > toler and alphas[i] > 0):

                # 选取[i, m]范围内的随机数预测类型、计算偏差
                j = selectJrand(i, m)
                fXj = float(multiply(alphas, labelMat).T * (dataMatrix * dataMatrix[j, :].T)) + b
                Ej = fXj - float(labelMat[j])

                # 比较当前样本点与随机样本点，求alpha最优修改量
                alphaIold, alphaJold = alphas[i].copy(), alphas[j].copy()       # 保留旧alpha
                ...
                if abs(alphas[j] - alphaJold) < 0.00001:    # 检查alpha[j]发生轻微改变则跳过循环
                    print("j not moving enough")
                    continue
                alphas[i] += labelMat[j] * labelMat[i] * (alphaJold - alphas[j])    # update i by the same amount as j

                #  alphas[i]与alphas[j]朝相反方向更新
